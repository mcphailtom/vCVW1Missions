...
function foxAir:OnAfterEnterSafeZone(From, Event, To, Player)
    ...
    MESSAGE:New( player.name .. " you have entered the Joshua missile safe zone" ):ToClient( player.client )

    dcsbot.sendBotMessage(player.name .. ' has entered the Joshua missile safe zone')
end

function foxAir:OnAfterExitSafeZone(From, Event, To, Player)
    local player = Player

    MESSAGE:New( player.name .. " you have left the Joshua missile safe zone" ):ToClient( player.client )

    dcsbot.sendBotMessage(player.name .. ' has left the Joshua missile safe zone')
end
